...
def combineIntroMain(makingStage2, makingStage7, makingStage8):
    if not os.path.exists(makingStage8):
        os.makedirs(makingStage8)
...
def addWaterMark(folder_path, waterMark):
    ...
    if video_file:
        ...
        ffmpeg_command = (
            f"ffmpeg -i {input_video_path} -i {waterMark} "
            f"-filter_complex [1]scale=iw/2.5:ih/2.5[scaled];[0][scaled]overlay=10:(H-h)/2 "
            f"{output_video_path}"
        )
